...
When the `agents` repo is cloned as a sibling of the dotfiles repo (e.g. `~/git/dotfiles` and `~/git/agents`), the dotfiles installer wires it in automatically. The `my-private-repo` repo follows the same sibling convention.
