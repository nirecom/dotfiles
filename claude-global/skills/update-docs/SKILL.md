...
- **my-specs-repo projects** (langchain-stack, litellm-stack, open-webui-stack, portable-llm-server, llama-swap, judgeclaw):
  - Docs directory: `../my-specs-repo/projects/engineering/` (subdirectory per project: `langchain/`, `llama-swap/`, `judgeclaw/`, etc.)
  - If not found locally, clone from `git@github.com:nirecom/my-specs-repo.git`
...
6. **Propagate to parent docs**: For my-specs-repo projects, update parent-level counterparts
   (e.g. `langchain/todo.md` → `engineering/todo.md`). Skip for repo-local `docs/`.
7. **Commit separately**: If docs are in a separate repo (e.g., my-specs-repo), commit each repo independently
