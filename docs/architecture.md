...
- **`codes` function**: Opens VS Code in a new window (`--new-window`), polls for window closure via title matching (`wait-vscode-window.ps1` / `.sh`), then runs session-sync push. Each instance independently detects its own window, so closing one window triggers push even while other windows remain open. Windows: Win32 EnumWindows API. Linux: xdotool/wmctrl. macOS: osascript. Push runs in quiet mode (`-Quiet` / `--quiet`): success is silent, failure shows an OS notification (Windows: MessageBox, Linux: notify-send)
