...
| Starship install guard | `install.ps1 -Full` tried to reinstall Starship even when already installed — `Get-Command` missed binaries not yet in PATH | Added `Test-Path` check for winget default install path (`Program Files\starship\bin\starship.exe`), matching AutoHotkey's dual-check pattern (`Get-Command` OR `Test-Path`) | TBD |
