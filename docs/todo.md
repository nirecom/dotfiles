...
**再現条件**: `git -C "c:/LLM/my-specs-repo" commit` のように `-C` フラグのパスを引用符で囲むと、
`resolveRepoDir()` の regex `(\S+)` が `"c:/LLM/my-specs-repo"` を引用符付きのまま取得し、
...
回避策: `-C` のパスを引用符なしで渡す（`git -C c:/LLM/my-specs-repo commit ...`）。

**修正候補**: `resolveRepoDir` で取得したパス文字列の前後の `"` / `'` を strip する。

---

### Fix: my-specs-repo の history.md が Edit/Write deny で追記不能

`settings.json` の deny リストに `Edit(**/history.md)` / `Write(**/history.md)` があり、
`doc-append.py` 経由のみ許可する設計だが、my-specs-repo に `bin/doc-append.py` が存在しないため
history.md への追記が完全にブロックされる。

対処案:
- my-specs-repo に `bin/doc-append.py` を追加 (dotfiles の同スクリプトをコピー or シンボリックリンク)
- または my-specs-repo 用の allow ルールを settings.json に追加
