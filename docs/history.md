...
Background: Document structure rules were only in my-specs-repo history.md (mixed with migration history) — not auto-loaded by Claude Code
...
Background: `git commit` in my-specs-repo (docs-only repo) was blocked by PreToolUse hooks (check-test-updated.js, check-docs-updated.js) because `.md` files under `projects/` were classified as "source code".
...
Changes:
  ...
  - `install-obsolete.ps1`: added cleanup for `~/dotfiles`, `~/my-private-repo`, `~/git`
...
Cause: Claude Code migrated session storage format from `UUID.jsonl` (flat file) to `UUID/subagents/` (directory structure). windows-host pushed both old and new formats on 3/29. When TEC ran `session-sync push` on 4/2, Claude Code had already deleted the old `.jsonl` files locally, so `git add .` staged 35 deletions and propagated them to remote.
...
Background: history.md had inconsistent ordering across projects (some ascending, some descending) and split Change/Incident sections that broke chronological flow. Research confirmed ascending order is optimal for LLM context processing (recency bias > primacy bias in deep transformer layers; context compression preserves later content). Research documented in my-specs-repo `engineering/research-results/llm-document-ordering.md`.
...
Changes: Added `claude-global/skills/save-research/SKILL.md`. The skill saves conversation research findings to `../my-specs-repo/projects/engineering/research-results/<slug>.md` using relative paths (dotfiles is a public repo). Follows the established format of the existing `llm-document-ordering.md` research file.
...
Changes: Added ALLOWLIST_PRIVATE loading from ../my-private-repo/.private-info-allowlist when present — environment-specific exceptions can live in the private repo without touching the public file. Added glob matching for file-scoped allowlist patterns (e.g., tests/*:@example.com), consolidating per-file entries into single glob lines.

### git config: work email isolation via my-private-repo (2026-04-11, (pending))
...
Changes: Added .config/git/config-work to global gitignore. my-private-repo now owns config.local (OS-specific settings + includeIf for ~/work/**) and config-work (work email). install/win/dotfileslink.ps1 added to my-private-repo to symlink both files into dotfiles/.config/git/.
...
Changes: Added README.md to Standard Files table (role: public-facing entry point, size: compact, when: public repos). Added content rules: external-facing overview, delegate internals to other docs, source repo root for my-specs-repo projects. Promoted README.md to always-target in /update-docs General projects; added source repo root target to my-specs-repo projects.

### Language policy: move rules to my-private-repo (2026-04-11, (pending))
...
Changes: Removed language rules from `claude-global/rules/coding.md` and `docs-convention.md`. Created `my-private-repo/claude-global/rules/language.md` (conversation, commit, code, docs language policy). Added gitignore entry and symlink support in both install scripts (win/linux). `install.ps1` and `install.sh` now auto-call `my-private-repo/install.ps1`/`install.sh` when present, passing the same flags.

### Rize installer: move to my-private-repo (2026-04-11, (pending))
...
Changes: Moved `install/win/rize.ps1` and `install/linux/rize.sh` to `my-private-repo/install/`. Removed Rize steps from `install.ps1` (Step 8) and `install.sh` (Step 7), renumbered subsequent steps. Added Rize steps to `my-private-repo/install.ps1` (Step 3, under `-Base`/`-Full`) and `my-private-repo/install.sh` (Step 2, under `--base`/`--full`). Moved tests (`main-rize.Tests.ps1`, `main-rize.sh`) to `my-private-repo/tests/`. Updated `docs/architecture.md` (removed file table entries and execution order references).
...
### my-private-repo: startup fetch in .profile_common (2026-04-11, pending)
Background: my-private-repo is a sibling git repo managed alongside dotfiles, but had no automatic pull on shell startup. Separately, "model": "sonnet" was added as a default model to claude-global/settings.json.
Changes: Added my-private-repo fetch block to .profile_common, after the dotfiles fetch and before the session-sync fetch. Added "model": "sonnet" to claude-global/settings.json.
...
### test rules: cross-platform temporary test file location (2026-04-11, pending)
Background: test.md had a rule directing temporary test files to `/tmp/tests/`, but this was macOS/Linux-specific. As a global rule it would cause Claude Code on Windows to also attempt writing to `/tmp/tests/`, which doesn't exist. Windows uses project-internal `tests/` instead.
Changes: Replaced single-line `/tmp/tests/` rule in claude-global/rules/test.md with a platform-conditional table (macOS/Linux → `/tmp/tests/`, Windows → `tests/`). Added `Write(/tmp/tests/**)` and `Edit(/tmp/tests/**)` to settings.json allow list.
