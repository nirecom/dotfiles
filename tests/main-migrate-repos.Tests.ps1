...
Describe "migrate-repos.ps1" {
    ...
    It "moves ~/my-private-repo to destination when it exists" {
        $src = Join-Path $script:FakeHome "my-private-repo"
        New-Item -ItemType Directory -Path $src -Force | Out-Null
        Set-Content -Path (Join-Path $src "secret.txt") -Value "private"

        & $MigrateScript -HomeDir $script:FakeHome -DestDir $script:FakeDest
        Test-Path $src | Should -BeFalse
        Test-Path (Join-Path $script:FakeDest "my-private-repo\secret.txt") | Should -BeTrue
    }
