...
| Blocklist patterns | User-defined in `my-private-repo/.private-info-blocklist` | Hostnames, domain names |
...
An optional private allowlist (`../my-private-repo/.private-info-allowlist`) is also loaded
for environment-specific patterns. When `my-private-repo/` is absent, only the local allowlist is used.
...
The blocklist lives in the sibling private repo (`my-private-repo/.private-info-blocklist`)
