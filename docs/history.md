...
Fix: Removed `export` prefix in session-start.js. Tightened test 30 to use `grep -qx` (exact line match). E2E verification of the full chain (CLAUDE_ENV_FILE → CLAUDE_SESSION_ID injected into Bash tool) still pending — new session required.
