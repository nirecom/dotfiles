...
Background: commit 6caa366 (2026-04-11) added a prompt in install-obsolete to delete ~/.gitconfig because it was overriding the dotfiles-managed XDG git config and causing private emails to leak into commits. Later, the agents installer (commit d8b7ee7) intentionally changed core.hooksPath write destination to ~/.gitconfig. Reason: config.local is a symlink to my-private-repo/config.local.linux (tracked), so writing an absolute path there dirties a tracked file; --global falls back to ~/.config/git/config (dotfiles-managed, also tracked) under XDG precedence. ~/.gitconfig is untracked by any repo and is the correct place for machine-local per-user config. This created a conflict: dotfiles install-obsolete would delete what agents intentionally wrote.
...
Changes: Added bin/colors.sh shared color utility (C_GRAY=\033[0;90m, TTY-detected). install.sh now sources it instead of inline definitions. dotfileslink.sh gains a link_file() function that prints Already linked in gray, Relinking in yellow, and Linked in green, replacing blind ln -sf calls. keychain.sh and nvm.sh print already installed messages in gray. my-private-repo/install/linux/dotfileslink.sh updated to use the same color scheme via sourced colors.sh.
...
### CONFIG: Move dotfiles.code-workspace to my-private-repo (2026-04-30, -)
Background: dotfiles.code-workspace existed in both repos; my-private-repo had a stale 2-folder version
Changes: Removed from dotfiles repo; my-private-repo now has the canonical 4-folder layout (dotfiles, my-private-repo, agents, fornix)

### FEATURE: installer: migrate install hub to my-private-repo (2026-04-30, pending)
Background: dotfiles/install.{ps1,sh} was the install chain hub, calling my-private-repo and agents at the end. This inverted the ownership: the public repo was driving the private one. Migrated the hub role to my-private-repo/install.{ps1,sh}, which clones sibling repos (dotfiles, agents, fornix) via clone_if_missing / Initialize-Repo helpers if absent, then calls each installer in order. dotfiles/install.{ps1,sh} is now self-contained. IS_DOTFILES_SLAVE env var lets dotfiles/install.sh skip its own exec $SHELL -l when called from the hub, so the hub can continue to agents/fornix after dotfiles completes.
Changes: Removed my-private-repo and agents chain calls from dotfiles/install.ps1 and install.sh. Added IS_DOTFILES_SLAVE guard around exec $SHELL -l in install.sh. Rewrote my-private-repo/install.{ps1,sh} as hub with Initialize-Repo / clone_if_missing helpers. Created my-private-repo/docs/architecture.md documenting the hub design.
...
Background: my-private-repo install.ps1 clones fornix-agent as a sibling repo, but profile.ps1 did not include it in the parallel fetch list on PowerShell startup.
Changes: install/win/profile.ps1: added $FornixAgentDir variable and fornix-agent fetch/merge block alongside dotfiles, my-private-repo, agents, and session-sync.
...
Background: my-private-repo's install.sh needs a way to inject machine-specific env vars (e.g. AWS_WORK_DIR) without modifying the public dotfiles repo directly. A sourcing hook in .profile_common provides a stable extension point for private overrides.
Changes: .profile_common: added sourcing of ~/.config/dotfiles/local.env (if it exists) immediately after DOTFILES_DIR is confirmed. File is generated by my-private-repo install.sh.
...
### REFACTOR: Symmetrize Node version manager installers — install manager only, no Node version (2026-05-11, pending)
Background: install/linux/nvm.sh installed nvm AND `nvm install --lts` + `nvm alias default lts/*`, while install/win/fnm.ps1 installed fnm only. The asymmetry meant Linux/macOS got a Node version "as of install day" that drifted with time (the `lts/*` alias resolves to the latest *locally installed* LTS, not the current upstream LTS), while Windows had no Node at all. Both behaviours conflicted with the per-repo version policy (`.node-version` / `.nvmrc` in each project). install/linux/nodenv-node.sh was a legacy anyenv+nodenv path that no installer chain called.
Changes: install/linux/nvm.sh: removed `nvm install --lts`, `nvm alias default lts/*`, and the post-install `nvm setup complete` echo. Added a policy comment block explaining that the manager is installed but Node is each repo's responsibility. Deleted install/linux/nodenv-node.sh (legacy, unreferenced). README.md: clarified that no-flag install sets up a version manager only — Node itself is not installed. docs/architecture/file-tree.md: updated nvm.sh row description.

### REFACTOR: Replace 500ms polling in force-japanese-layout.ahk with event-driven shell hook (2026-05-19, #13)
Background: force-japanese-layout.ahk used an unconditional 500ms polling loop that posted WM_INPUTLANGCHANGEREQUEST on every tick with no debouncing, flooding the foreground window's message queue and causing CPU wake activity every 500ms — suspected to contribute to all-key keyboard input slowness/chattering after several hours of uptime.
Changes: Rewrote the script to use RegisterShellHookWindow + OnMessage on WM_SHELLHOOK, dispatching HSHELL_WINDOWACTIVATED (uses the event's lParam HWND directly to avoid the WinGetID race) and HSHELL_LANGUAGE. Split CheckLayout into CheckLayout()/CheckLayoutForHwnd(hwnd). Added a CheckLayout() startup call to correct any non-Japanese layout active at launch, plus a 15s safety-net SetTimer covering startup races and delivery gaps. Guards: RegisterWindowMessage=0 and RegisterShellHookWindow failure both fall back to timer-only mode via OutputDebug.

### FEATURE: PR #327 — fix/wsl-fetch-passphrase (2026-07-03, d871d61, #327)
Background: fix(profile): prevent WSL terminal hang on SSH passphrase prompt
Changes: BUGFIX: WSL interactive shell hang after SSH passphrase prompt (#326). The startup background `git fetch` in `.profile_common` (`_dotfiles_fetch_all`) launched ssh without non-interactive flags, so ssh read `/dev/tty` for a key passphrase, absorbed terminal input, and blocked the interactive `wait` — the terminal went unresponsive on WSL login. Wrapped all three fetches (dotfiles, my-private-repo, agents) in subshells exporting `GIT_TERMINAL_PROMPT=0 GIT_SSH_COMMAND='ssh -o BatchMode=yes'` so ssh fails fast instead of prompting (fetch failure → merge skipped → shell stays responsive). Also fixed the pure-zsh session guard — `pgrep -fo bash` returns empty under zsh, so `[ -n "$PID" ]` was false and the entire fetch block was skipped; added `|| [ -n "${ZSH_VERSION-}" ]` so zsh sessions always run the block while the bash path is unchanged. Replaced GNU-only `find -printf '%f\n'` with a POSIX `find | while read | basename` pipeline for macOS/BSD portability of the SSH key auto-detection. Test gap: no runtime/L3 test exercises real ssh reading /dev/tty during an interactive WSL login or the terminal staying responsive after `wait`; coverage is grep-based source assertions in main-git-fetch-sync.sh plus find_keys behavioral tests in main-keychain-ssh-agent.sh. <!-- compose-doc-append-sentinel: branch=fix/wsl-fetch-passphrase pr=#327 -->

### FEATURE: PR #334 — fix/333-fix-profile-common-windows-git (2026-08-31, 8671e556a903ada16af4b82270d46b29457f8c41, #334)
Background: fix(.profile_common): repair Windows Git Bash startup fetch
Changes: fix(.profile_common): Windows Git Bash startup fetch was broken by three compounding bugs — `agents/profile-snippet.sh` sourced before the SSH-setup block undid its own OpenSSH `PATH` priority, three fetch-progress `echo` statements polluted stdout, and a `pgrep`-dependent dedup guard silently skipped the fetch on Windows where `pgrep` is unreliable/absent. Fixed by moving the source block after SSH setup, redirecting the progress echoes to stderr, and adding an explicit `$OSDIST = "mingw"` branch that bypasses the dedup guard (with the pre-existing stale-`PID` leak also cleared). `GIT_SSH_COMMAND` needed no change — the source-order fix resolved it automatically. Verified via `tests/main-git-fetch-sync.sh` (30/30) and the new `tests/fix-profile-common-startup-fetch.sh` (55/55). Accepted tradeoff: concurrent mingw sessions may still duplicate-fetch, since the dedup guard is fully bypassed on Windows rather than replaced. (#333) <!-- compose-doc-append-sentinel: branch=fix/333-fix-profile-common-windows-git pr=#334 -->

### FEATURE: PR #337 — feature/335-cc-claude-code-bash-session (2026-09-02, 2dc79d803a836b5a0927cb70c1b9963fb358750f, #337)
Background: fix(.profile_common): skip session-sync fetch for Claude Code bash sh...
Changes: fix(#335): `.profile_common`'s session-sync `git fetch` on bash startup now skips when `CLAUDECODE` is set and stdout is not a TTY, so shells launched by Claude Code's Bash tool (always non-interactive) no longer trigger the fetch; human-interactive bash shells are unaffected. Added `tests/fix-profile-common-startup-fetch.sh` (plus `guard-behaviour.sh` and `full-pipeline-integration.sh` parts) covering the CLAUDECODE + non-TTY skip matrix. Filed #336 as a related follow-up: dotfiles lacks `tests/run-all.sh`, so the centralized `/run-tests` RUN_CONTRACT auto-completion mechanism does not work in this repo and required a manual sentinel bypass during this session. <!-- compose-doc-append-sentinel: branch=feature/335-cc-claude-code-bash-session pr=#337 -->
