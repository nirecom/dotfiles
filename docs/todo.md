...
research 結果: `my-specs-repo/projects/engineering/research-results/claude-code-workflow-step-enforcement.md` 参照。
PostToolUse marker interception 安定後に着手する。
既知バグ (#37210, #18312) への対処が必要。

### Workflow State Machine — macOS/Linux 動作確認

Windows での E2E 確認は完了済み。macOS または Linux 環境（mac-host-1 等）で同等の確認が必要。
