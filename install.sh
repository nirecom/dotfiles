...
# Run my-private-repo installer if available (sibling repo)
PRIVATE_INSTALLER="$(dirname "$DOTFILES_DIR")/my-private-repo/install.sh"
if [ -x "$PRIVATE_INSTALLER" ]; then
    echo ""
    printf "${C_BOLD}--- Running my-private-repo installer ---${C_RESET}\n"
    "$PRIVATE_INSTALLER" "$@"
fi
