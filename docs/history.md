...
### Security Enhancement Phase 4: Trojan Source detection + scan-outbound rename (2026-04-18, pending)
...
### workflow-mark: && chaining support + workflow step rename (2026-04-20, (pending))
...
### Fix workflow-gate: quoted -C path causes hasStagedDocChanges false-negative + fail-open bypass (2026-04-20, pending)
Background: resolveRepoDir() used \S+ regex to extract git -C <arg>, capturing quoted paths verbatim (e.g. "c:/LLM/my-specs-repo" with surrounding quotes). execSync received an invalid cwd, making hasStagedDocChanges/hasStagedTestChanges always return false — gate blocked even when docs/tests were staged. Deeper issue: the same \S+ in commit detection stopped at the first space in "c:/path with space", causing !commitMatch to fire and approve() to run — silently bypassing all gate checks (fail-open).
Changes: New claude-global/hooks/lib/parse-git-args.js with parseGitCArg() — quote-aware parser handling bare, double-quoted, and single-quoted -C arguments; unterminated quotes return null. workflow-gate.js: resolveRepoDir delegates to parseGitCArg; commit detection refactored to 2-step (/^git\s/ + /\scommit(\s|$)/) to prevent \S+ regex bypass. is-private-repo.js: extractRepoDirFromCommand delegates to parseGitCArg. resolveRepoDir adds c:/path -> c:\path normalization on win32. Tests: tests/fix-workflow-gate-unix-path.sh extended with Sections D-G (34 cases total, all PASS).
