...
# 15. ~/.claude/rules/language.md exists as a symlink resolving to my-private-repo
...
if [ -L "$LIVE_LINK" ]; then
    ...
    case "$target" in
        *my-private-repo/claude-global/rules/language.md)
            ...
            ;;
        *)
            echo "FAIL: symlink target is '$target' (expected ending with my-private-repo/claude-global/rules/language.md)"
            fail=$((fail + 1))
            ;;
    esac
else
    ...
    fail=$((fail + 1))
    echo "FAIL: symlink resolves to my-private-repo/claude-global/rules/language.md (skipped — link missing)"
    fail=$((fail + 1))
fi
