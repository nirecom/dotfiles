...
3. **hook 相対 path 調整**: `claude-global/hooks/pre-commit` と `commit-msg` の `../../bin/scan-outbound.sh` → `"$AGENTS_CONFIG_DIR/../bin/scan-outbound.sh"`（過渡期。step 7 以降は `$AGENTS_CONFIG_DIR/bin/` 配下）。`bin/scan-outbound.sh` L14-15 を `${DOTFILES_PRIVATE_DIR:-$DOTFILES_DIR/../my-private-repo}` に。
