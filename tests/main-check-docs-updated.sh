...
# --- my-specs-repo fallback integration tests ---
# Helper: set up a pair of repos (source + sibling my-specs-repo)
setup_repo_pair() {
    local base="$TMPDIR_BASE/pair-$RANDOM"
    local repo="$base/llm-infra-check"
    local aispecs="$base/my-specs-repo"
...
echo ""
echo "=== my-specs-repo fallback: external docs staged (should approve) ==="
...
echo ""
echo "=== my-specs-repo fallback: external docs unstaged (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== my-specs-repo fallback: no external docs changes (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== my-specs-repo fallback: no sibling my-specs-repo dir (should block) ==="
...
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
expect_block_repo "no sibling my-specs-repo" "$REPO" "$COMMIT_JSON"

echo ""
echo "=== my-specs-repo fallback: same-name dir not found in my-specs-repo (should block) ==="
PAIR_BASE="$TMPDIR_BASE/pair-$RANDOM"
REPO="$PAIR_BASE/some-other-repo"
AISPECS="$PAIR_BASE/my-specs-repo"
...
echo ""
echo "=== my-specs-repo fallback: no code changes, exempt only (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
mkdir -p "$REPO/tests"
echo "test" > "$REPO/tests/test.sh"
git -C "$REPO" add tests/test.sh
expect_approve_repo "exempt only, my-specs-repo sibling exists" "$REPO" "$COMMIT_JSON"

echo ""
echo "=== my-specs-repo fallback: changes outside same-name dir (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== my-specs-repo fallback: multiple matching dirs, one has changes (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== my-specs-repo fallback: filename with spaces (should block without docs) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
...
echo ""
echo "=== my-specs-repo fallback: filename with spaces + external docs (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/my app.js"
git -C "$REPO" add "src/my app.js"
echo "updated" >> "$AISPECS/projects/engineering/llm-infra-check/architecture.md"
...
echo ""
echo "=== my-specs-repo fallback: external docs already committed recently (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
echo "updated" >> "$AISPECS/projects/engineering/llm-infra-check/architecture.md"
...
echo ""
echo "=== my-specs-repo fallback: external docs committed in wrong dir (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
mkdir -p "$AISPECS/projects/engineering/other-project"
...
echo ""
echo "=== my-specs-repo fallback: external docs committed long ago (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== my-specs-repo fallback: multiple dirs, one has recent commit (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
mkdir -p "$AISPECS/archive/llm-infra-check"
...
echo ""
echo "=== my-specs-repo fallback: local docs take priority over my-specs-repo (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_pair)"
mkdir -p "$REPO/docs"
echo "# history" > "$REPO/docs/history.md"
git -C "$REPO" add docs/
...
# --- docs/.my-specs-repo marker tests ---

# Helper: set up repo pair with .my-specs-repo marker
setup_repo_with_marker() {
    ...
    git -C "$repo" config user.name "Test"
    echo "init" > "$repo/README.md"
    echo "$marker_content" > "$repo/docs/.my-specs-repo"
    git -C "$repo" add -A
    git -C "$repo" commit -q -m "initial"
...
echo ""
echo "=== docs/.my-specs-repo marker: changes in mapped project (should approve) ==="
...
echo ""
echo "=== docs/.my-specs-repo marker: no changes in mapped project (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== docs/.my-specs-repo marker: unstaged changes in mapped project (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
...
echo ""
echo "=== docs/.my-specs-repo marker: recent commits in mapped project (should approve) ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
echo "updated" >> "$AISPECS/projects/engineering/langchain/architecture.md"
...
echo ""
echo "=== docs/.my-specs-repo marker: fallback without marker (should use repo name) ==="
...
echo ""
echo "=== docs/.my-specs-repo marker: whitespace/newline trimmed ==="
...
echo ""
echo "=== docs/.my-specs-repo marker: empty file (should fall back to repo name) ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker open-webui-stack "EMPTY")"
: > "$REPO/docs/.my-specs-repo"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
expect_block_repo "marker: empty file, falls back to repo name" "$REPO" "$COMMIT_JSON"

echo ""
echo "=== docs/.my-specs-repo marker: project not found in my-specs-repo (should block) ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker open-webui-stack "nonexistent-project")"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
expect_block_repo "marker: project not found in my-specs-repo" "$REPO" "$COMMIT_JSON"

echo ""
echo "=== docs/.my-specs-repo marker: local docs change takes priority ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker)"
echo "new code" > "$REPO/src/app.js"
...
echo ""
echo "=== docs/.my-specs-repo marker: idempotency ==="
read -r REPO AISPECS <<< "$(setup_repo_with_marker)"
echo "new code" > "$REPO/src/app.js"
git -C "$REPO" add src/app.js
result1=$(run_hook_in_repo "$REPO" "$COMMIT_JSON")
