...
| `instruction.md` | **SSOT** for physical machines, network, Docker stacks, ports, and cloud resources per stack/host. Other docs must reference this — never duplicate host placements. | Unlimited | Always (in `my-specs-repo`) |
...
file at `docs/.my-specs-repo` containing the logical project name:

```
langchain
```

The hook reads this file and searches `my-specs-repo` for a directory matching that
