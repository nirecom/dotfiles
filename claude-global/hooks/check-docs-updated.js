...
//   2. Sibling ../my-specs-repo repo — find same-name directory, check for changes
...
// Check sibling ../my-specs-repo for same-name directory with changes
function checkAiSpecsDocs(repoDir) {
  const resolvedRepo = path.resolve(repoDir);
  const aiSpecsDir = path.join(path.dirname(resolvedRepo), "my-specs-repo");

  if (!fs.existsSync(aiSpecsDir)) return false;

  // Check for docs/.my-specs-repo marker file (overrides repo name matching)
  const markerFile = path.join(resolvedRepo, "docs", ".my-specs-repo");
  let projectName;
  try {
...
// Priority 2: sibling ../my-specs-repo with same-name directory
