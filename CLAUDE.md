...
- **Claude Code config:** `claude-global/` stores global Claude Code settings. `CLAUDE.md` is a pointer to `rules/` (behavioral rules) and `commands/` (skills). Symlinked to `~/.claude/` by install scripts. Named `claude-global/` (not `.claude/`) to avoid conflict with project-level settings
