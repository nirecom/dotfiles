...
### [Bug] workflow-gate の hasStagedTestChanges が Unix スタイルパスを解決できない — 要修正

**再現手順:**
1. Git Bash から `git -C <unix-style-path> commit ...` で commit を実行 (例: ドライブレター Unix 形式)
2. PreToolUse フック (workflow-gate.js) が `resolveRepoDir` でパスを抽出
3. `hasStagedTestChanges(repoDir)` が `execSync("git diff --cached --name-only", { cwd: repoDir })` を実行
4. Windows Node.js は Unix スタイルの cwd を受け付けない → 例外が `catch` で握りつぶされ `false` を返す
5. tests/ がステージ済みでも `write_tests` がクリアされず commit ブロック

**観察した挙動:** Windows ネイティブ形式に変えても同様にブロック。staged tests/ の証拠ベースチェックが機能しない。

**回避策（実施済み）:** `node -e` で `markStep` を直接呼び出してステートファイルを書き換え。

**修正候補:**
- [ ] `resolveRepoDir` で Unix スタイルドライブパス → Windows ネイティブパスへの正規化を追加
- [ ] `hasStagedTestChanges` の `execSync` 失敗時に stderr を警告ログへ出力（サイレント失敗を防止）
